...
class PEMIRL(RLAlgorithm):
    ...
    Args:
        ...
        sampler (garage.sampler.Sampler): Sampler used to sample trajectories from 
            the environment(s).
        ...
        num_initial_steps (int): Number of transitions to initialize context-encoder.
        meta_batch_size (int): Meta batch size - number of tasks per iteration.
        batch_size (int): Number of trajectories to obtain per task.
        use_information_bottleneck (bool): False means latent context is
            deterministic.
        replay_buffer_size (int): Maximum samples in buffer.
        discount (float): RL discount factor.
        info_coeff (float): Info coefficient, used to scale the loss.
        imitation_coeff (float): Used to scale latent-loss in initialization of
            context-encoder.
        airl_iter (int): Number of iterations to train discriminator per iteration.
        state_only (bool): Whether or not to use next state instead of action for
            expert trajectories.
        policy_ent_coeff (float): Policy Entropy Coefficient.
        use_softplus_entropy (bool): Whether or not to use softplus function on 
            entropy regulizer.
        stop_entropy_gradient (bool): Whether or not the loss is back-propagated
            through the entropy regulizer. 
        gae_lambda (float): Lambda value for Generalized Advantage Estimation.
        center_adv (bool): Whether or not to center advantages on zero.
        positive_adv(bool) Whether or not advantages should only be positive.
        """
    ...
    aug_obs = akro.Box(low=np.tile(np.concatenate((
     self._single_env.spec.observation_space.low, 
     self._single_env.spec.action_space.low)), self.T),
     high=np.tile(np.concatenate((self._single_env.spec.observation_space.high, 
                                  self._single_env.spec.action_space.high)), self.T),
     dtype=np.float32)
    ...
    ### Opimizers for AIRL ###
    central_opt_params = list(self._vf.parameters()) +\
    list(self._reward_encoder.parameters()) +\
    list(self._policy.networks[0].parameters()) +\
    list(self._policy.networks[1].parameters())
  ...
  def _pre_train(self, batch_size=400, kl_weight=0.1):
      ...
      for i in range(self._num_initial_steps):
       ...
       #Get tasks for this training iteration
       indices = np.random.choice(range(self._num_train_tasks),
                                   batch_size)

       #Sample expert demonstrations for this batch
       exp_obs, _, exp_acts, _ = self._sample_experts(indices)

       exp_obs = exp_obs.view(-1, self.O)
       exp_acts = exp_acts.view(-1, self.A)

       exp_traj = torch.cat((exp_obs, exp_acts), dim=-1)
       exp_traj = exp_traj.view(-1, self.T*(self.O+self.A))
       dist, _ = self._policy.context_forward(exp_traj)
       z = dist.rsample()

       #Probability of observing latent variable in prior distribution
       log_pz = self.log_normal_pdf(z.view(-1, self._latent_dim), 
           torch.zeros(dist.mean.size(), device=global_device()), 
           torch.zeros(dist.variance.size(), device=global_device()))

       #Probability of observing latent variable in posterior distribution
       log_qz = self.log_normal_pdf(z.view(-1, self._latent_dim), 
        dist.mean, dist.variance.log())

       #Repeat latent variable across trajectory
       z_tile = z.view(-1, self._latent_dim).unsqueeze(1).repeat(
        1, self.T, 1).view(-1, self._latent_dim)
       ...
       #if (i % 100 == 0):
       #    print(
       #        "Pretrain Epoch", i, "PolicyLikelihood", 
       #        np.mean(policy_likelihood_loss.cpu().detach().numpy()), "KL",  
       #        np.mean(latent_loss.cpu().detach().numpy()), "Total", 
       #        np.mean(loss.cpu().detach().numpy())
       #        )
  ...
  def _train_airl(self, obs, next_obs, actions_log_prob, 
      ...
      r"""Train Discriminator.
      Args:
          obs(torch.Tensor): Observations from the environment
              with shape :math:`(X*N, T, O*)`.
          next_obs (torch.Tensor): Next observation from the environment
              with shape :math:`(X*N, T, O*)`.
          actions_log_prob (torch.Tensor): Log probability of actions fed
              to the environment with shape :math:`(X x N, T, 1)`.
          exp_obs (torch.Tensor): Expert observations
              with shape :math:`(X*N, T, O*)`.
          exp_next_obs (torch.Tensor): Expert next observations
              with shape :math:`(X*N, T, O*)`.
          exp_actions (torch.Tensor): Expert actions
              with shape :math:`(X*N, T, A*)`.
          exp_action_log_prob (torch.Tensor): Log probability of
              expert actions with shape :math:`(X*N, T, 1)`.
          exp_traj_batch (torch.Tensor): Expert trajectory batch
              used to infer latent variable, shape :math:`(X*N, T, A* x O*)`.
      """
      ...
      for _ in range(self.airl_itr):
       ...
       exp_obs_batch, exp_next_obs_batch, \
       exp_actions_batch, exp_action_log_prob_batch = self.sample_batch(
       ...
       self._train_airl_once(
           exp_traj_batch, obs_batch, next_obs_batch, 
           actions_log_prob_batch, exp_obs_batch, 
           exp_next_obs_batch, exp_actions_batch, 
  ...
  def _train_airl_once(self, exp_traj_batch, obs, next_obs, actions_log_prob, 
      ...
      Args:
          exp_traj_batch (torch.Tensor): Expert trajectory batch
              used to infer latent variable, shape :math:`(N, T, A* x O*)
          obs(torch.Tensor): Observations from the environment
              with shape :math:`(N, T, O*)`.
          next_obs (torch.Tensor): Next observation from the environment
              with shape :math:`(N, T, O*)`.
          actions_log_prob (torch.Tensor): Log probability of actions fed
              to the environment with shape :math:`(N, T, 1)`.
          exp_obs (torch.Tensor): Expert observations
              with shape :math:`(N, T, O*)`.
          exp_next_obs (torch.Tensor): Expert next observations
              with shape :math:`(N, T, O*)`.
          exp_actions (torch.Tensor): Expert actions
              with shape :math:`(N, T, A*)`.
          exp_action_log_prob (torch.Tensor): Log probability of
              expert actions with shape :math:`(N, T, 1)`.
      """

      #Repeat across batch dimension
      exp_traj_batch_tile = (exp_traj_batch.view(self._meta_batch_size, 1, 
          self.T, self.O + self.A)).repeat(1, self._batch_size, 1, 1)
      ...
      expert_traj_batch_input = torch.cat([exp_traj_batch_tile, 
                                torch.cat([exp_obs, exp_actions], axis=-1)\
      ...
      labels = torch.zeros((self._meta_batch_size, self._batch_size*2, 1, 1), 
                            device=global_device())
      labels[:, self._batch_size:, ...] = 1.0

      #Infer latent variable
      dist, _ = self._policy.context_forward(expert_traj_batch_input.view(
       -1, self.T*(self.O + self.A)))
      z = dist.rsample()
      ...
      z = z.view(self._meta_batch_size, self._batch_size*2, 
          self._latent_dim).unsqueeze(2).repeat(1, 1, self.T, 1)

      #Infer Reward
      z = z.view(-1, self._latent_dim)
      obs_in = torch.cat((obs_t.view(-1, self.O), z), dim=1)
      reward = self._reward_encoder(obs_in)
      #Sum reward over trajectories
      sampled_traj_return = torch.sum(reward.view(
       self._meta_batch_size, -1, self.T), axis=-1, keepdims=True)
      ...
      imitation_obses = exp_traj_batch.view(
       self._meta_batch_size, 1, self.T, -1)[..., :self.O]
      imitation_acts = exp_traj_batch.view(
       self._meta_batch_size, 1, self.T, -1)[..., self.O:]
      ...
      log_p_tau = (reward  + fitted_value_fn_n - fitted_value_fn).view(
       self._meta_batch_size, -1, self.T, 1)
      log_q_tau = lprobs

      #Discriminator loss
      log_pq = torch.stack((log_p_tau, log_q_tau)).logsumexp(axis=0)
      #discrim_output = torch.exp(log_p_tau - log_pq)
      cent_loss = -(labels*(log_p_tau-log_pq) +\
       (1-labels)*(log_q_tau-log_pq)).mean()
      ...
      # Used for computing the gradient w.r.t. omega
  ...
  def _eval_airl(self, obs, next_obs, actions, actions_log_prob, exp_traj):
      ...
      Args:
          ...
          actions_log_prob (torch.Tensor): Log probability of actions fed
              to the environment with shape :math:`(N, T, 1)`.
          exp_traj (torch.Tensor): Expert trajecory
              with shape :math:`(N, T, A* x O*)`.
      Returns:
          rewards (torch.Tensor): Rewards from the reward-encoder for
              each time-step, shape: :math:`(X, N, T, 1)`.
      """
      ...
      with torch.no_grad():
       ...
       z_probs = dist.log_prob(z)
       #Repeat latent variable across trajectory dimension
       self.z = z.view(self._meta_batch_size, self._batch_size, 
           self._latent_dim).unsqueeze(2).repeat(1, 1, self.T, 1)

       #All trajectories are from sampled data, so true label = 0
       labels = torch.zeros((self._meta_batch_size, 
           self._batch_size, 1, 1), device=global_device())

       obs_in = torch.cat([obs.view(self._meta_batch_size, 
           self._batch_size, self.T, self.O), self.z], dim=-1)
       nobs_in = torch.cat([next_obs.view(self._meta_batch_size, 
           self._batch_size, self.T, self.O), self.z], dim=-1)
       ...
       """ Evaluate losses - for logging purposes only """
       ...
       #Define losses
       log_p_tau = (reward  + fitted_value_fn_n - fitted_value_fn)
       log_p_tau = log_p_tau.view(self._meta_batch_size, -1, self.T, 1)
       ...
       # Used for computing gradient w.r.t. psi
       info_loss = -(log_q_m_tau * (1 - squeezed_labels)).mean() / (1 - labels).mean()
       # Used for computing gradient w.r.t. omega
       info_surr_loss = - torch.mean(
           (1 - squeezed_labels) * log_q_m_tau.detach() * sampled_traj_return -
           (1 - squeezed_labels) * log_q_m_tau.detach() * 
           torch.mean(sampled_traj_return*(1-squeezed_labels), axis=1, keepdims=True) /
           (1-labels).mean()) / (1-labels).mean()

       loss = cent_loss + self.info_coeff*info_loss

       imitation_z = self.z.view(self._meta_batch_size, 
           self._batch_size, self.T, -1)[:,0,...].contiguous().view(-1, self._latent_dim)
       ...
       with tabular.prefix(prefix + '/'):
           tabular.record('AverageDiscrim-Output', 
            np.mean(discrim_output.cpu().detach().numpy()))
           tabular.record('AverageCentral-Loss', 
            np.mean(cent_loss.cpu().detach().numpy()))
           tabular.record('AverageInfo-Loss', 
            np.mean(info_loss.cpu().detach().numpy()))
           tabular.record('AverageInfo-Surrogate-Loss', 
            np.mean(info_surr_loss.cpu().detach().numpy()))
           tabular.record('AveragePolicy-Loss', 
            np.mean(policy_likelihood_loss.cpu().detach().numpy()))
  ...
  def _train_policy(self, obs, actions, advantages):
      ...
      loss = self._compute_loss_with_adv(
       obs.cpu(), actions.cpu(), advantages.cpu()
       )
      loss.backward()
      self._policy_optimizer.step(
          f_loss=lambda: self._compute_loss_with_adv(obs.cpu(), \
           actions.cpu(), advantages.cpu()),
  ...
  def _obtain_samples(self,
      ...
      """Obtain samples from the environment using current policy.
      Args:
  ...
  def _sample_data(self, indices):
      ...
      Returns:
          Obervations (torch.Tensor), with shape :math:`(X, N, T, O^*)` where X
              is the number of tasks. N is batch size. T is max episode length.
          Next obervations (torch.Tensor), with shape :math:`(X, N, T, O^*)`.
          Actions (torch.Tensor), with shape :math:`(X, N, T, A^*)`.
          Next actions (torch.Tensor), with shape :math:`(X, N, T, A^*)`.
          Actions_log_prob (torch.Tensor), log-probability of observing the actions
              under current policy distribution, shape :math:`(X, N, T, 1)`.
          Rewards (torch.Tensor), with shape :math:`(X, N, T, 1)`.
          Succeses (torch.Tensor), Whether or not the agent is at the goal position,
              shape :math:`(X, N, T, 1)`.
          Lengths (torch.Tensor), length of trajectory
  ...
  def _sample_experts(self, indices, n=1):
      ...
      Args:
          indices (list): List of task indices to sample from.
          n (int): Number of trajectories to sample per task.
      Returns:
          ...
          torch.Tensor: Reward, with shape :math:`(X, N, T, 1)`.
      """
  ...
  def log_normal_pdf(self, sample, mean, logvar, raxis=1):
      ...
      return torch.sum(
          -.5 * ((sample - mean) ** 2. * torch.exp(-logvar) + 
